...
"""
1958 Multi数据集图像提取器
使用训练好的模型对1958 Multi文件夹中的所有图像进行处理
提取检测到的图像区域并保存到"1958 Multi_Extracted_Dataset"文件夹
专为多Issue数据集优化，包含详细的Issue分析和质量评估
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/Users/zhaoye/Desktop/1956_TI_Dataset/ti_1958_multi_extraction.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

class TI1958MultiExtractor:
    ...
    def create_comprehensive_metadata(self, processing_results: Dict[str, Any]) -> Path:
        """创建综合数据集元数据"""
        logger.info("📋 创建1958 Multi综合数据集元数据...")
        ...
        metadata = {
            'dataset_info': {
                'name': '1958 Multi Extracted Dataset',
                'description': '从1958 Multi TI文档中提取的多Issue图像数据集，涵盖12个不同Issue的综合分析',
    ...
    def _analyze_year_characteristics(self) -> Dict[str, Any]:
        ...
        return {
            'year': '1958',
            'historical_context': '1958年是TI公司重要发展年份',
            'issue_diversity': len(self.stats['issue_distribution']),
            'data_richness': 'high' if self.stats['total_detections'] > 200 else 'medium',
    ...
    def _generate_comprehensive_recommendations(self) -> List[str]:
        ...
        if top_tier_ratio > 0.7:
            recommendations.extend([
                "1958年高质量多Issue数据集，适合专业深度学习项目",
                "顶级和卓越置信度图像可作为核心训练集",
                "适合用于历史文档处理模型的训练和验证"
            ])
        elif top_tier_ratio > 0.5:
            recommendations.extend([
                "1958年优质多Issue数据集，质量可靠",
                "建议优先使用高质量图像进行训练"
            ])
        else:
            recommendations.extend([
        ...
        recommendations.extend([
            f"数据来源于{issue_count}个不同的1958年Issue，具有良好的内容多样性",
            "建议根据Issue进行分层采样和交叉验证",
            "可用于研究1958年不同Issue间的差异性",
            "适合构建历史文档图像检测训练集",
            "支持基于年份的时间序列分析"
        ])
    ...
    def _create_comprehensive_readme(self, metadata: Dict[str, Any], readme_file: Path):
        ...
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("# 1958 Multi Extracted Dataset\n\n")
            ...
            f.write("## 文件夹结构\n\n")
            f.write("```\n")
            f.write("1958 Multi_Extracted_Dataset/\n")
            ...
            f.write("示例：`1958_Issue-1_1958_Issue-1_42-43_page_0_fig0_fig1_extracted_1_premium_conf0.956.jpg`\n\n")
            
            f.write("---\n")
            f.write(f"*1958年多Issue数据集生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n")
    ...
    def _get_issue_recommendations(self) -> Dict[str, str]:
        ...
        for i, issue_data in enumerate(rankings):
            ...
            if issue_data['quality_score'] > 80:
                recommendations[issue] = "1958年优秀Issue，推荐优先使用"
            elif issue_data['quality_score'] > 60:
                recommendations[issue] = "1958年良好Issue，适合常规使用"
            else:
                recommendations[issue] = "1958年一般Issue，建议结合其他数据使用"
    ...
    def _get_quality_recommendations(self) -> Dict[str, List[str]]:
        """获取质量使用建议"""
        return {
            'premium_quality': ["1958年黄金标准图像，直接用于关键应用"],
            'excellent_confidence': ["1958年顶级质量图像，推荐生产环境使用"],
            'high_confidence': ["1958年优秀质量图像，适合大多数应用"],
            'good_confidence': ["1958年良好质量图像，适合一般应用"],
            'low_confidence': ["需要人工审核后使用"]
        }
    
    def run_comprehensive_extraction_pipeline(self) -> Dict[str, Any]:
        """运行综合提取pipeline"""
        logger.info("🚀 启动1958 Multi综合数据集提取Pipeline")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("🎉 1958 Multi综合数据集提取Pipeline执行成功!")
...
def main():
    ...
    source_dir = "/Users/zhaoye/Desktop/1958 Multi"
    output_dir = "/Users/zhaoye/Desktop/1958 Multi_Extracted_Dataset"
    
    print("🔍 1958 Multi综合数据集提取Pipeline")
    print("=" * 70)
    print(f"🎯 使用模型: {Path(model_path).name}")
    print(f"📂 源目录: {source_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("🎨 功能: 中等规模处理 + 5级分类 + 12个Issue深度分析 + 1958年特征研究")
    print("=" * 70)
    
    try:
        # 创建综合数据集提取器
        extractor = TI1958MultiExtractor(model_path, source_dir, output_dir)
        
        # 运行完整pipeline
        results = extractor.run_comprehensive_extraction_pipeline()
        
        if results['success']:
            print("\n🎉 1958 Multi综合数据集提取Pipeline执行成功!")
            print("=" * 70)
            ...
            print("5. 中等规模1958年数据集，适合历史文档处理研究")
