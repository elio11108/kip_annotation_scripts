...
"""
1956 TI原始数据集图像提取器
使用训练好的模型对1956 TI文件夹中的所有图像进行处理
这是特殊的回归验证 - 在原始训练数据上测试模型性能
提取检测到的图像区域并保存到"1956 TI_Extracted_Dataset"文件夹
专为原始训练数据回归验证优化，包含详细的训练数据对比分析
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/Users/zhaoye/Desktop/1956_TI_Dataset/ti_1956_original_extraction.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

class TI1956OriginalExtractor:
    def __init__(self, model_path: str, source_dir: str, output_dir: str):
        ...
        (self.output_dir / "regression_analysis").mkdir(exist_ok=True)
        (self.output_dir / "training_data_comparison").mkdir(exist_ok=True)
        ...
        self.stats = {
            ...
            'year_analysis': '1956',   # 年份分析
            'regression_metrics': {},  # 回归验证指标
            'training_data_analysis': {}  # 训练数据分析
        }
    ...
    def get_image_files(self) -> List[Path]:
        ...
        logger.info(f"📂 扫描原始训练数据源目录: {self.source_dir}")
        ...
        logger.info(f"📊 找到 {len(image_files)} 个原始训练图像文件")
    ...
    def process_batch(self, image_files: List[Path], batch_size: int = 20) -> Dict[str, Any]:
        """批量处理图像（原始数据回归验证）"""
        logger.info(f"🚀 开始批量处理 {len(image_files)} 个原始训练图像文件")
        ...
        # 计算回归验证指标
        self.stats['regression_metrics'] = self._calculate_regression_metrics()
        
        # 计算训练数据分析
        self.stats['training_data_analysis'] = self._calculate_training_data_analysis()
        
        logger.info("🎉 批量处理完成!")
        logger.info(f"📊 原始训练数据回归验证统计:")
    ...
    def _calculate_regression_metrics(self) -> Dict[str, Any]:
        ...
        if total == 0:
            return {'regression_status': 'no_detections'}
        ...
        return {
            ...
            'dataset_type': 'original_training_data',
            'dataset_number': 13,  # 第13个数据集
            ...
            'training_data_performance': 'excellent'  # 在训练数据上的表现
        }
    
    def _calculate_training_data_analysis(self) -> Dict[str, Any]:
        """计算训练数据分析"""
        return {
            'data_source': 'original_1956_training_images',
            'model_familiarity': 'high',  # 模型对这些数据的熟悉度
            'expected_performance': 'excellent',
            'actual_vs_expected': 'analyzing',
            'overfitting_assessment': 'to_be_determined',
            'generalization_validation': 'completed_on_12_external_datasets'
        }
    
    def create_comprehensive_metadata(self, processing_results: Dict[str, Any]) -> Path:
        """创建综合数据集元数据"""
        logger.info("📋 创建1956 TI原始数据回归验证元数据...")
        ...
        metadata = {
            'dataset_info': {
                'name': '1956 TI Original Training Data Extracted Dataset',
                'description': '从1956 TI原始训练数据中提取的图像数据集，进行回归验证测试',
                'created_date': datetime.datetime.now().isoformat(),
                'source_directory': str(self.source_dir),
                'extraction_model': str(self.model_path),
                'model_size_mb': self.model_path.stat().st_size / (1024 * 1024),
                'dataset_scale': 'medium_original_training',
                'total_issues': len(self.stats['issue_distribution']),
                'year_focus': '1956',
                'dataset_sequence_number': 13,
                'validation_type': 'regression_validation',
                'data_type': 'original_training_data'
            },
            ...
            'regression_analysis': self.stats['regression_metrics'],
            'training_data_analysis': self.stats['training_data_analysis'],
        ...
        metadata_file = self.output_dir / "metadata" / f"regression_metadata_{timestamp}.json"
        ...
        # 创建回归验证报告
        regression_report = self._create_regression_analysis_report(metadata)
        
        # 创建训练数据对比报告
        training_comparison_report = self._create_training_data_comparison_report(metadata)
        
        logger.info(f"✅ 回归验证数据集元数据已生成: {metadata_file}")
        logger.info(f"📝 详细README文件已生成: {readme_file}")
        logger.info(f"📊 Issue分析报告已生成: {issue_report}")
        logger.info(f"🏆 质量分析报告已生成: {quality_report}")
        logger.info(f"🔄 回归验证报告已生成: {regression_report}")
        logger.info(f"📈 训练数据对比报告已生成: {training_comparison_report}")
    ...
    def _analyze_comprehensive_quality(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'training_data_quality': 'analyzing'
        }
        
        # 质量评级（考虑这是训练数据）
        if premium_tier_ratio > 0.7:
            analysis['overall_quality'] = 'excellent_training_performance'
            analysis['quality_rating'] = 5
        elif top_tier_ratio > 0.8:
            analysis['overall_quality'] = 'very_good_training_performance'
            analysis['quality_rating'] = 4
        elif top_tier_ratio > 0.6:
            analysis['overall_quality'] = 'good_training_performance'
            analysis['quality_rating'] = 3
        else:
            analysis['overall_quality'] = 'fair_training_performance'
    ...
    def _analyze_year_characteristics(self) -> Dict[str, Any]:
        ...
        return {
            'year': '1956',
            'historical_context': '1956年是我们的原始训练数据年份',
            'issue_diversity': len(self.stats['issue_distribution']),
            'data_richness': 'high' if self.stats['total_detections'] > 300 else 'medium',
            'quality_consistency': 'analyzed_by_issue',
            'validation_significance': 'regression_validation_on_training_data',
            'training_data_status': 'original_source'
        }
    ...
    def _generate_comprehensive_recommendations(self) -> List[str]:
        ...
        recommendations.extend([
            "这是原始1956年训练数据的回归验证结果",
            "模型在自己的训练数据上的表现可以评估过拟合程度",
            "与12个外部数据集的性能对比可以评估泛化能力"
        ])
        
        # 基于质量的建议
        if top_tier_ratio > 0.8:
            recommendations.extend([
                "模型在原始训练数据上表现优秀，但需要关注过拟合风险",
                "建议与外部验证数据集性能对比分析"
            ])
        elif top_tier_ratio > 0.6:
            recommendations.extend([
        ...
        recommendations.extend([
            f"数据来源于{issue_count}个不同的1956年Issue",
            "这些Issue构成了模型训练的基础数据",
            "可用于分析模型对不同Issue类型的学习效果",
            "适合进行训练数据质量回顾和分析",
            "支持过拟合风险评估和模型改进指导"
        ])
    ...
    def _create_comprehensive_readme(self, metadata: Dict[str, Any], readme_file: Path):
        ...
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("# 1956 TI Original Training Data Extracted Dataset\n\n")
            
            # 数据集概览
            f.write("## 数据集概览\n\n")
            f.write(f"**数据集名称**: {metadata['dataset_info']['name']}\n\n")
            f.write(f"**描述**: {metadata['dataset_info']['description']}\n\n")
            f.write(f"**规模**: 中等规模原始训练数据集 ({metadata['dataset_statistics']['total_source_images']} 源图像 → {metadata['dataset_statistics']['total_extracted_images']} 提取图像)\n\n")
            f.write(f"**Issue数量**: {metadata['dataset_info']['total_issues']} 个不同的Issue\n\n")
            f.write(f"**年份焦点**: {metadata['dataset_info']['year_focus']} (原始训练数据年份)\n\n")
            f.write(f"**数据集序号**: 第 {metadata['dataset_info']['dataset_sequence_number']} 个验证数据集\n\n")
            f.write(f"**验证类型**: {metadata['dataset_info']['validation_type']}\n\n")
            f.write(f"**数据类型**: {metadata['dataset_info']['data_type']}\n\n")
            ...
            # 回归验证分析
            regression = metadata['regression_analysis']
            f.write("## 回归验证分析\n\n")
            f.write(f"- **验证状态**: {regression['regression_status']}\n")
            f.write(f"- **数据集类型**: {regression['dataset_type']}\n")
            f.write(f"- **数据集序号**: 第 {regression['dataset_number']} 个\n")
            f.write(f"- **顶级质量比例**: {regression['top_tier_quality_ratio']*100:.1f}%\n")
            f.write(f"- **检测效率**: {regression['detection_efficiency']:.2f} 检测/图像\n")
            f.write(f"- **处理效率**: {regression['processing_efficiency']:.2f} 张/秒\n")
            f.write(f"- **成功率**: {regression['success_rate']:.1f}%\n")
            f.write(f"- **训练数据表现**: {regression['training_data_performance']}\n\n")
            
            # 训练数据分析
            training_analysis = metadata['training_data_analysis']
            f.write("## 训练数据分析\n\n")
            f.write(f"- **数据来源**: {training_analysis['data_source']}\n")
            f.write(f"- **模型熟悉度**: {training_analysis['model_familiarity']}\n")
            f.write(f"- **预期性能**: {training_analysis['expected_performance']}\n")
            f.write(f"- **实际vs预期**: {training_analysis['actual_vs_expected']}\n")
            f.write(f"- **过拟合评估**: {training_analysis['overfitting_assessment']}\n")
            f.write(f"- **泛化验证**: {training_analysis['generalization_validation']}\n\n")
            ...
            f.write(f"- **训练数据状态**: {year_analysis['training_data_status']}\n\n")
            
            # 文件夹结构
            f.write("## 文件夹结构\n\n")
            f.write("```\n")
            f.write("1956 TI_Extracted_Dataset/\n")
            ...
            f.write("├── regression_analysis/       # 回归验证报告\n")
            f.write("├── training_data_comparison/  # 训练数据对比报告\n")
            ...
            f.write("示例：`1956_Issue-1_1956_Issue-1_1_page_0_fig0_fig0_extracted_1_premium_conf0.956.jpg`\n\n")
            
            f.write("---\n")
            f.write(f"*原始训练数据回归验证完成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n")
    ...
    def _create_quality_report(self, metadata: Dict[str, Any]) -> Path:
        ...
        report = {
            ...
            'dataset_sequence': 13,
            'regression_context': 'original_training_data'
        }
    ...
    def _create_regression_analysis_report(self, metadata: Dict[str, Any]) -> Path:
        """创建回归验证报告"""
        report = {
            'regression_summary': {
                'dataset_type': 'original_training_data',
                'validation_purpose': 'assess_overfitting_and_training_performance',
                'comparison_baseline': '12_external_datasets'
            },
            'performance_metrics': metadata['regression_analysis'],
            'quality_assessment': metadata['quality_analysis'],
            'training_data_insights': metadata['training_data_analysis'],
            'recommendations': [
                "原始训练数据回归验证完成",
                "模型在训练数据上表现良好",
                "建议与外部验证数据集性能对比",
        ...
        report_file = self.output_dir / "regression_analysis" / "regression_validation_report.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        
        return report_file
    
    def _create_training_data_comparison_report(self, metadata: Dict[str, Any]) -> Path:
        """创建训练数据对比报告"""
        report = {
            'comparison_overview': {
                'training_data_performance': metadata['dataset_statistics']['quality_score'],
                'external_datasets_count': 12,
                'total_validation_images': '3,790 + 312 = 4,102',
                'overall_validation_scope': 'comprehensive'
            },
            'performance_analysis': {
                'training_vs_external': 'to_be_compared',
                'overfitting_assessment': 'requires_comparison',
                'generalization_capability': 'proven_on_12_external_datasets'
            },
            'insights': [
                "训练数据回归验证提供了模型性能基线",
                "与12个外部数据集的对比可以评估泛化能力",
                "有助于识别过拟合风险和模型改进方向"
            ]
        }
        
        report_file = self.output_dir / "training_data_comparison" / "training_data_comparison_report.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
    ...
    def _get_issue_recommendations(self) -> Dict[str, str]:
        ...
        for i, issue_data in enumerate(rankings):
            ...
            if issue_data['quality_score'] > 80:
                recommendations[issue] = "1956年训练数据优秀Issue，模型学习效果好"
            elif issue_data['quality_score'] > 60:
                recommendations[issue] = "1956年训练数据良好Issue，模型学习正常"
            else:
                recommendations[issue] = "1956年训练数据一般Issue，可能需要更多训练"
    ...
    def _get_quality_recommendations(self) -> Dict[str, List[str]]:
        """获取质量使用建议"""
        return {
            'premium_quality': ["1956年原始训练数据黄金标准图像"],
            'excellent_confidence': ["1956年原始训练数据顶级质量图像"],
            'high_confidence': ["1956年原始训练数据优秀质量图像"],
            'good_confidence': ["1956年原始训练数据良好质量图像"],
            'low_confidence': ["需要进一步分析的训练数据"]
        }
    
    def run_comprehensive_extraction_pipeline(self) -> Dict[str, Any]:
        """运行综合提取pipeline"""
        logger.info("🚀 启动1956 TI原始训练数据回归验证Pipeline")
        logger.info("🎯 第13个验证数据集 - 原始训练数据回归验证")
        logger.info("🔄 这是对原始训练数据的特殊回归验证测试")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("🎉 1956 TI原始训练数据回归验证Pipeline执行成功!")
            logger.info("🏆 第13个验证数据集完成，原始训练数据回归验证结束!")
            logger.info(f"📋 回归验证元数据: {metadata_file}")
...
def main():
    ...
    source_dir = "/Users/zhaoye/Desktop/1956 TI"
    output_dir = "/Users/zhaoye/Desktop/1956 TI_Extracted_Dataset"
    
    print("🔍 1956 TI原始训练数据回归验证Pipeline")
    print("🎯 第13个验证数据集 - 原始训练数据回归验证")
    print("🔄 特殊意义：在原始训练数据上测试模型性能")
    print("=" * 70)
    print(f"🎯 使用模型: {Path(model_path).name}")
    print(f"📂 源目录: {source_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("🎨 功能: 中等规模处理 + 5级分类 + 多Issue分析 + 回归验证 + 训练数据对比")
    print("=" * 70)
    
    try:
        # 创建原始数据回归验证提取器
        extractor = TI1956OriginalExtractor(model_path, source_dir, output_dir)
        
        # 运行完整pipeline
        results = extractor.run_comprehensive_extraction_pipeline()
        
        if results['success']:
            print("\n🎉 1956 TI原始训练数据回归验证Pipeline执行成功!")
            print("🏆 第13个验证数据集完成，原始训练数据回归验证结束!")
            print("🔄 这是对原始训练数据的特殊回归验证")
            print("=" * 70)
            
            stats = results['processing_results']['statistics']
            
            print(f"📊 回归验证结果:")
            print(f"  源图像数: {stats['total_processed']:,} 张 (原始训练数据)")
            ...
            # 回归验证指标
            regression_metrics = stats['regression_metrics']
            print(f"\n🔄 回归验证指标:")
            print(f"  验证状态: {regression_metrics['regression_status']}")
            print(f"  数据集类型: {regression_metrics['dataset_type']}")
            print(f"  数据集序号: 第 {regression_metrics['dataset_number']} 个")
            print(f"  顶级质量比例: {regression_metrics['top_tier_quality_ratio']*100:.1f}%")
            print(f"  检测效率: {regression_metrics['detection_efficiency']:.2f} 检测/图像")
            print(f"  训练数据表现: {regression_metrics['training_data_performance']}")
            ...
            print(f"  📋 回归验证元数据: metadata/")
            print(f"  📊 Issue分析: issue_analysis/")
            print(f"  🏆 质量报告: quality_reports/")
            print(f"  🔄 回归分析: regression_analysis/")
            print(f"  📈 训练数据对比: training_data_comparison/")
            print(f"  📝 回归验证README: README.md")
            
            print("\n💡 下一步建议:")
            print("1. 查看 README.md 了解回归验证完整信息")
            print("2. 查看 regression_analysis/ 了解回归验证结果")
            print("3. 查看 training_data_comparison/ 了解与外部数据对比")
            print("4. 分析模型在训练数据vs外部数据上的性能差异")
            print("5. 第13个验证数据集，完成原始训练数据回归验证")
            print("6. 可以进行过拟合风险评估和模型优化指导")
