...
"""
1958 TI终极数据集图像提取器
处理最大规模的1958 TI文件夹，包含两个子文件夹的复杂结构
使用训练好的模型对所有图像进行处理，确保零遗漏
提取检测到的图像区域并保存到"1958 TI_Extracted_Dataset"文件夹
专为大规模复杂结构数据集优化，包含完整的子文件夹映射和质量分析
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/Users/zhaoye/Desktop/1956_TI_Dataset/ti_1958_ultimate_extraction.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

class TI1958UltimateExtractor:
    def __init__(self, model_path: str, source_dir: str, output_dir: str):
        ...
        self.stats = {
            ...
            'cross_subfolder_analysis': {} # 跨子文件夹分析
        }
        ...
        try:
            ...
            if subfolder_name not in self.stats['subfolder_distribution']:
                self.stats['subfolder_distribution'][subfolder_name] = {
                    ...
                    'confidence_breakdown': {'premium': 0, 'excellent': 0, 'high': 0, 'good': 0, 'medium': 0, 'low': 0}
                }
    ...
    def process_batch(self, image_files: List[Path], batch_size: int = 25) -> Dict[str, Any]:
        ...
        logger.info(f"🎯 置信度阈值: 顶级≥{self.confidence_thresholds['premium']}, 卓越≥{self.confidence_thresholds['excellent']}, 高≥{self.confidence_thresholds['high']}, 良好≥{self.confidence_thresholds['good']}, 中等≥{self.confidence_thresholds['medium']}, 低≥{self.confidence_thresholds['low']}")
        ...
        if total_detections > 0:
            ...
            self.stats['dataset_quality_score'] = (
                premium_ratio * 1.0 + excellent_ratio * 0.95 + high_ratio * 0.8 + 
                good_ratio * 0.6 + medium_ratio * 0.35 + low_ratio * 0.15
            ) * 100
    ...
    def create_ultimate_metadata(self, processing_results: Dict[str, Any]) -> Path:
        ...
        metadata = {
            'dataset_info': {
                'name': '1958 TI Ultimate Extracted Dataset',
                'description': '从1958 TI文档中提取的终极规模图像数据集，包含两个子文件夹的完整递归处理',
            ...
            if stats['processed'] > 0:
                ...
                if total_detections > 0:
                    ...
                    quality_score = (
                        ...
                        conf_breakdown['excellent'] / total_detections * 0.95 +
                        conf_breakdown['high'] / total_detections * 0.8 +
                        conf_breakdown['good'] / total_detections * 0.6 +
    ...
    def _generate_ultimate_recommendations(self) -> List[str]:
        ...
        if top_tier_ratio > 0.7:
            recommendations.extend([
                "终极规模世界级数据集，适合大规模深度学习项目",
                "顶级和卓越置信度图像可作为黄金标准训练集",
                "适合用于模型预训练和高精度基准测试"
            ])
        elif top_tier_ratio > 0.5:
            recommendations.extend([
    ...
    def _create_ultimate_readme(self, metadata: Dict[str, Any], readme_file: Path):
        ...
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("# 1958 TI Ultimate Extracted Dataset\n\n")
            ...
            f.write("## 文件夹结构\n\n")
            f.write("```\n")
            f.write("1958 TI_Extracted_Dataset/\n")
            ...
            f.write("示例：`1958_1_1958_Issue-1_44_page_0_fig0_fig0_extracted_1_premium_conf0.956.jpg`\n\n")
            
            f.write("---\n")
            f.write(f"*终极规模双子文件夹数据集生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n")
    ...
    def _get_subfolder_recommendations(self) -> Dict[str, str]:
        ...
        for i, subfolder_data in enumerate(rankings):
            ...
            if subfolder_data['quality_score'] > 80:
        ...
        for subfolder, stats in self.stats['subfolder_distribution'].items():
            if stats['detections'] > 0:
                ...
                comparison[subfolder] = {
                    ...
                    'high_ratio': conf_breakdown['high'] / total,
                    'top_tier_ratio': (conf_breakdown['premium'] + conf_breakdown['excellent'] + conf_breakdown['high']) / total
                }
    ...
    def _get_ultimate_quality_recommendations(self) -> Dict[str, List[str]]:
        """获取终极质量使用建议"""
        return {
            'premium_quality': ["黄金标准图像，直接用于最关键应用"],
    ...
    def run_ultimate_extraction_pipeline(self) -> Dict[str, Any]:
        """运行终极提取pipeline"""
        logger.info("🚀 启动1958 TI终极数据集提取Pipeline")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("🎉 1958 TI终极数据集提取Pipeline执行成功!")
...
def main():
    ...
    source_dir = "/Users/zhaoye/Desktop/1958 TI"
    output_dir = "/Users/zhaoye/Desktop/1958 TI_Extracted_Dataset"
    
    print("🔍 1958 TI终极数据集提取Pipeline")
    print("=" * 70)
    print(f"🎯 使用模型: {Path(model_path).name}")
    print(f"📂 源目录: {source_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("🎨 功能: 终极规模处理 + 6级分类 + 双子文件夹递归 + 完整映射分析")
    print("=" * 70)
    
    try:
        # 创建终极数据集提取器
        extractor = TI1958UltimateExtractor(model_path, source_dir, output_dir)
        
        # 运行完整pipeline
        results = extractor.run_ultimate_extraction_pipeline()
        
        if results['success']:
            print("\n🎉 1958 TI终极数据集提取Pipeline执行成功!")
            print("=" * 70)
            ...
            print("4. 根据子文件夹分布进行分层采样")
            print("5. 终极规模数据集，适合大规模深度学习训练和研究")
