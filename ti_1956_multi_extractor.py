...
"""
1956 Multi数据集图像提取器
使用训练好的模型对1956 Multi文件夹中的所有图像进行处理
提取检测到的图像区域并保存到"1956 Multi_Extracted_Dataset"文件夹
专为1956年Multi数据集优化，这是1956年系列的收官之作
第十四个验证数据集，完成1956年完整验证系列
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/Users/zhaoye/Desktop/1956_TI_Dataset/ti_1956_multi_extraction.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

class TI1956MultiExtractor:
    def __init__(self, model_path: str, source_dir: str, output_dir: str):
        ...
        (self.output_dir / "year_1956_analysis").mkdir(exist_ok=True)
        ...
        self.stats = {
            ...
            'series_completion_metrics': {}  # 系列完成指标
        }
    ...
    def get_image_files(self) -> List[Path]:
        ...
        logger.info(f"📂 扫描1956 Multi源目录: {self.source_dir}")
        ...
        logger.info(f"📊 找到 {len(image_files)} 个1956 Multi图像文件")
    ...
    def process_batch(self, image_files: List[Path], batch_size: int = 15) -> Dict[str, Any]:
        """批量处理图像（小规模精品优化）"""
        logger.info(f"🚀 开始批量处理 {len(image_files)} 个1956 Multi图像文件")
        ...
        # 计算系列完成指标
        self.stats['series_completion_metrics'] = self._calculate_series_completion_metrics()
        
        logger.info("🎉 批量处理完成!")
        logger.info(f"📊 1956 Multi处理统计:")
    ...
    def _calculate_series_completion_metrics(self) -> Dict[str, Any]:
        """计算系列完成指标"""
        total = self.stats['total_detections']
        if total == 0:
            return {'completion_status': 'no_detections'}
        ...
        return {
            'completion_status': 'series_completed',
            'dataset_number': 14,  # 第14个数据集
            'year_series_status': '1956_complete',  # 1956年系列完成
    ...
    def create_comprehensive_metadata(self, processing_results: Dict[str, Any]) -> Path:
        """创建综合数据集元数据"""
        logger.info("📋 创建1956 Multi综合数据集元数据...")
        ...
        metadata = {
            'dataset_info': {
                'name': '1956 Multi Extracted Dataset',
                'description': '从1956 Multi TI文档中提取的多Issue图像数据集，1956年系列的收官之作',
                'created_date': datetime.datetime.now().isoformat(),
                'source_directory': str(self.source_dir),
                'extraction_model': str(self.model_path),
                'model_size_mb': self.model_path.stat().st_size / (1024 * 1024),
                'dataset_scale': 'small_multi_issue',
                'total_issues': len(self.stats['issue_distribution']),
                'year_focus': '1956',
                'dataset_sequence_number': 14,
                'year_series_status': '1956_completion',
                'validation_significance': 'completes_1956_year_series'
            },
            ...
            'series_completion_analysis': self.stats['series_completion_metrics'],
        ...
        # 创建1956年分析报告
        year_1956_report = self._create_year_1956_analysis_report(metadata)
        
        logger.info(f"✅ 1956 Multi综合数据集元数据已生成: {metadata_file}")
        logger.info(f"📝 详细README文件已生成: {readme_file}")
        logger.info(f"📊 Issue分析报告已生成: {issue_report}")
        logger.info(f"🏆 质量分析报告已生成: {quality_report}")
        logger.info(f"📅 1956年分析报告已生成: {year_1956_report}")
    ...
    def _analyze_year_characteristics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'validation_significance': 'completes_1956_year_series',
            'training_data_context': 'same_year_as_training_data'
        }
    ...
    def _generate_comprehensive_recommendations(self) -> List[str]:
        ...
        recommendations.extend([
            "这是1956年Multi数据集，完成1956年系列验证",
            "与原始训练数据同年份，可用于同年代数据性能对比",
            "第14个验证数据集，进一步验证模型稳定性"
        ])
        
        # 基于质量的建议
        if top_tier_ratio > 0.7:
            recommendations.extend([
                "1956年高质量多Issue数据集，适合专业应用",
                "顶级和卓越置信度图像可作为核心数据",
                "适合用于同年代文档处理研究"
            ])
        elif top_tier_ratio > 0.5:
            recommendations.extend([
                "1956年优质多Issue数据集，质量可靠",
                "建议优先使用高质量图像"
            ])
        ...
        recommendations.extend([
            ...
            "与原始训练数据同年份，可用于年代一致性研究",
            "适合构建1956年专门的图像检测数据集",
            "支持同年代数据的质量对比分析",
            "完成1956年Multi系列，与TI系列形成完整对比"
        ])
    ...
    def _create_comprehensive_readme(self, metadata: Dict[str, Any], readme_file: Path):
        ...
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("# 1956 Multi Extracted Dataset\n\n")
            
            # 数据集概览
            f.write("## 数据集概览\n\n")
            f.write(f"**数据集名称**: {metadata['dataset_info']['name']}\n\n")
            f.write(f"**描述**: {metadata['dataset_info']['description']}\n\n")
            f.write(f"**规模**: 小规模多Issue数据集 ({metadata['dataset_statistics']['total_source_images']} 源图像 → {metadata['dataset_statistics']['total_extracted_images']} 提取图像)\n\n")
            f.write(f"**Issue数量**: {metadata['dataset_info']['total_issues']} 个不同的Issue\n\n")
            f.write(f"**年份焦点**: {metadata['dataset_info']['year_focus']} (与原始训练数据同年)\n\n")
            f.write(f"**数据集序号**: 第 {metadata['dataset_info']['dataset_sequence_number']} 个验证数据集\n\n")
            f.write(f"**年份系列状态**: {metadata['dataset_info']['year_series_status']}\n\n")
            f.write(f"**验证意义**: {metadata['dataset_info']['validation_significance']}\n\n")
            ...
            # 系列完成分析
            series_completion = metadata['series_completion_analysis']
            f.write("## 1956年系列完成分析\n\n")
            f.write(f"- **完成状态**: {series_completion['completion_status']}\n")
            f.write(f"- **数据集序号**: 第 {series_completion['dataset_number']} 个\n")
            f.write(f"- **年份系列状态**: {series_completion['year_series_status']}\n")
            f.write(f"- **顶级质量比例**: {series_completion['top_tier_quality_ratio']*100:.1f}%\n")
            f.write(f"- **检测效率**: {series_completion['detection_efficiency']:.2f} 检测/图像\n")
            f.write(f"- **处理效率**: {series_completion['processing_efficiency']:.2f} 张/秒\n")
            f.write(f"- **成功率**: {series_completion['success_rate']:.1f}%\n\n")
            ...
            f.write(f"- **训练数据关联**: {year_analysis['training_data_context']}\n\n")
            
            # 文件夹结构
            f.write("## 文件夹结构\n\n")
            f.write("```\n")
            f.write("1956 Multi_Extracted_Dataset/\n")
            ...
            f.write("├── year_1956_analysis/        # 1956年分析报告\n")
            ...
            f.write("示例：`1956_Issue-1_1956_Issue-1_17_page_0_fig0_fig0_extracted_1_premium_conf0.956.jpg`\n\n")
            
            f.write("---\n")
            f.write(f"*1956年Multi系列收官数据集生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n")
    ...
    def _create_issue_analysis_report(self, metadata: Dict[str, Any]) -> Path:
        ...
        report = {
            ...
            'year_context': '1956',
            'dataset_sequence': 14,
            'series_context': '1956_completion'
        }
    ...
    def _create_quality_report(self, metadata: Dict[str, Any]) -> Path:
        ...
        report = {
            ...
            'dataset_sequence': 14,
            'year_series_context': '1956_completion'
        }
    ...
    def _create_year_1956_analysis_report(self, metadata: Dict[str, Any]) -> Path:
        """创建1956年分析报告"""
        report = {
            'year_1956_summary': {
                'training_data_dataset': 'completed',  # 1956 TI (训练数据)
                'multi_data_dataset': 'completed',     # 1956 Multi (当前)
                'total_1956_datasets': 2,
                'year_series_status': 'complete'
            },
            'year_comparison': {
                'training_vs_multi': 'performance_comparison_available',
                'same_year_consistency': 'to_be_analyzed'
            },
            'performance_metrics': metadata['series_completion_analysis'],
            'quality_assessment': metadata['quality_analysis'],
            'recommendations': [
                "1956年Multi数据集成功完成",
                "与1956 TI训练数据形成完整的1956年验证",
                "可用于同年代数据的一致性分析",
                "适合进行训练数据vs同年代外部数据对比"
            ]
        }
        
        report_file = self.output_dir / "year_1956_analysis" / "year_1956_analysis_report.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
    ...
    def _get_issue_recommendations(self) -> Dict[str, str]:
        ...
        for i, issue_data in enumerate(rankings):
            ...
            if issue_data['quality_score'] > 80:
                recommendations[issue] = "1956年优秀Issue，推荐优先使用"
            elif issue_data['quality_score'] > 60:
                recommendations[issue] = "1956年良好Issue，适合常规使用"
            else:
                recommendations[issue] = "1956年一般Issue，建议结合其他数据使用"
    ...
    def _get_quality_recommendations(self) -> Dict[str, List[str]]:
        """获取质量使用建议"""
        return {
            'premium_quality': ["1956年黄金标准图像，直接用于关键应用"],
            'excellent_confidence': ["1956年顶级质量图像，推荐生产环境使用"],
            'high_confidence': ["1956年优秀质量图像，适合大多数应用"],
            'good_confidence': ["1956年良好质量图像，适合一般应用"],
            'low_confidence': ["需要人工审核后使用"]
        }
    
    def run_comprehensive_extraction_pipeline(self) -> Dict[str, Any]:
        """运行综合提取pipeline"""
        logger.info("🚀 启动1956 Multi综合数据集提取Pipeline")
        logger.info("🎯 第14个验证数据集 - 1956年系列收官之作")
        logger.info("📅 与原始训练数据同年份的特殊验证")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("🎉 1956 Multi综合数据集提取Pipeline执行成功!")
            logger.info("🏆 第14个验证数据集完成，1956年系列圆满结束!")
            ...
            return {
                ...
                'year_1956_series_complete': True
            }
...
def main():
    ...
    source_dir = "/Users/zhaoye/Desktop/1956 Multi"
    output_dir = "/Users/zhaoye/Desktop/1956 Multi_Extracted_Dataset"
    
    print("🔍 1956 Multi综合数据集提取Pipeline")
    print("🎯 第14个验证数据集 - 1956年系列收官之作")
    print("📅 与原始训练数据同年份的特殊验证")
    print("=" * 70)
    print(f"🎯 使用模型: {Path(model_path).name}")
    print(f"📂 源目录: {source_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("🎨 功能: 小规模处理 + 5级分类 + 多Issue分析 + 1956年特征研究 + 系列收官")
    print("=" * 70)
    
    try:
        # 创建1956 Multi数据集提取器
        extractor = TI1956MultiExtractor(model_path, source_dir, output_dir)
        
        # 运行完整pipeline
        results = extractor.run_comprehensive_extraction_pipeline()
        
        if results['success']:
            print("\n🎉 1956 Multi综合数据集提取Pipeline执行成功!")
            print("🏆 第14个验证数据集完成，1956年系列圆满结束!")
            print("📅 完成与原始训练数据同年份的验证")
            print("=" * 70)
            ...
            # 系列完成指标
            series_metrics = stats['series_completion_metrics']
            print(f"\n📅 1956年系列完成指标:")
            print(f"  完成状态: {series_metrics['completion_status']}")
            print(f"  数据集序号: 第 {series_metrics['dataset_number']} 个")
            print(f"  年份系列状态: {series_metrics['year_series_status']}")
            print(f"  顶级质量比例: {series_metrics['top_tier_quality_ratio']*100:.1f}%")
            print(f"  检测效率: {series_metrics['detection_efficiency']:.2f} 检测/图像")
            print(f"  成功率: {series_metrics['success_rate']:.1f}%")
            ...
            print(f"  📅 1956年分析: year_1956_analysis/")
            ...
            print("4. 查看 year_1956_analysis/ 了解1956年系列分析")
            print("5. 第14个验证数据集，完成1956年系列验证")
            print("6. 可以进行1956年TI vs Multi的性能对比分析")
