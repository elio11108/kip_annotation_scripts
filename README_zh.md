...
> **说明**：本文件是项目开发期的原始中文工作笔记，按原样保留作为过程记录。其中提到的部分路径与文件（如 `setup.py`、`raw_images/`、`inference_results/`）反映的是当时的本地工作目录，并不存在于本仓库中。仓库的现行结构与使用方法请以英文 `README.md` 为准。另请注意：仓库随附的 `*.log` 中出现的 mAP 等评测数字是在存在重叠污染的数据切分上算出的，不代表模型的真实准确度，详见英文 `README.md` 的 "Data-split note"。
