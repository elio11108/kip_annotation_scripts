...
"""
1960 Multi数据集图像提取器
使用训练好的模型对1960 Multi文件夹中的所有图像进行处理
提取检测到的图像区域并保存到"1960 Multi_Extracted_Dataset"文件夹
专为中等规模数据集优化，包含详细的Issue分析
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/Users/zhaoye/Desktop/1956_TI_Dataset/ti_1960_multi_extraction.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

class TI1960MultiExtractor:
    def __init__(self, model_path: str, source_dir: str, output_dir: str):
        ...
        (self.output_dir / "high_confidence").mkdir(exist_ok=True)       # ≥0.6
        (self.output_dir / "good_confidence").mkdir(exist_ok=True)       # ≥0.4
    ...
    def process_batch(self, image_files: List[Path], batch_size: int = 20) -> Dict[str, Any]:
        ...
        if total_detections > 0:
            ...
            self.stats['dataset_quality_score'] = (
                premium_ratio * 1.0 + excellent_ratio * 0.9 + high_ratio * 0.7 + 
                good_ratio * 0.5 + low_ratio * 0.2
            ) * 100
    ...
    def create_comprehensive_metadata(self, processing_results: Dict[str, Any]) -> Path:
        """创建综合数据集元数据"""
        logger.info("📋 创建1960 Multi综合数据集元数据...")
        ...
        metadata = {
            'dataset_info': {
                'name': '1960 Multi Extracted Dataset',
                'description': '从1960 Multi TI文档中提取的中等规模图像数据集，包含多个Issue的综合分析',
        ...
        # 置信度分布
        total_detections = self.stats['total_detections']
        threshold_map = {'premium': '≥0.9', 'excellent': '≥0.8', 'high': '≥0.6', 'good': '≥0.4', 'low': '≥0.25'}
            ...
            if stats['processed'] > 0:
                ...
                if total_detections > 0:
                    ...
                    quality_score = (
                        ...
                        conf_breakdown['good'] / total_detections * 0.5 +
                        conf_breakdown['low'] / total_detections * 0.2
                    ) * 100
                else:
                    quality_score = 0
    ...
    def _generate_comprehensive_recommendations(self) -> List[str]:
        ...
        # 基于质量的建议
        if top_tier_ratio > 0.6:
            recommendations.extend([
                "中等规模高质量数据集，适合专业深度学习项目",
                "顶级和卓越置信度图像可作为核心训练集",
                "适合用于模型微调和性能基准测试"
            ])
        elif top_tier_ratio > 0.4:
        ...
        recommendations.extend([
            f"数据来源于{issue_count}个不同的Issue，具有良好的内容多样性",
    ...
    def _create_comprehensive_readme(self, metadata: Dict[str, Any], readme_file: Path):
        ...
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("# 1960 Multi Extracted Dataset\n\n")
            ...
            f.write("示例：`1960_Issue-1_1960_Issue-1_44_page_0_fig0_fig0_extracted_1_premium_conf0.956.jpg`\n\n")
            
            f.write("---\n")
            f.write(f"*中等规模多Issue数据集生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n")
    ...
    def _get_issue_recommendations(self) -> Dict[str, str]:
        ...
        for i, issue_data in enumerate(rankings):
            ...
            if issue_data['quality_score'] > 80:
                recommendations[issue] = "优秀Issue，推荐优先使用"
            elif issue_data['quality_score'] > 60:
                recommendations[issue] = "良好Issue，适合常规使用"
            else:
                recommendations[issue] = "一般Issue，建议结合其他数据使用"
    ...
    def _get_quality_recommendations(self) -> Dict[str, List[str]]:
        """获取质量使用建议"""
        return {
            'premium_quality': ["黄金标准图像，直接用于关键应用"],
            'excellent_confidence': ["顶级质量图像，推荐生产环境使用"],
    ...
    def run_comprehensive_extraction_pipeline(self) -> Dict[str, Any]:
        """运行综合提取pipeline"""
        logger.info("🚀 启动1960 Multi综合数据集提取Pipeline")
        logger.info("=" * 70)
        ...
        try:
            ...
            logger.info("🎉 1960 Multi综合数据集提取Pipeline执行成功!")
...
def main():
    ...
    source_dir = "/Users/zhaoye/Desktop/1960 Multi"
    output_dir = "/Users/zhaoye/Desktop/1960 Multi_Extracted_Dataset"
    
    print("🔍 1960 Multi综合数据集提取Pipeline")
    print("=" * 70)
    print(f"🎯 使用模型: {Path(model_path).name}")
    print(f"📂 源目录: {source_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("🎨 功能: 中等规模处理 + 5级分类 + Issue深度分析 + 综合质量评估")
    print("=" * 70)
    
    try:
        # 创建综合数据集提取器
        extractor = TI1960MultiExtractor(model_path, source_dir, output_dir)
        
        # 运行完整pipeline
        results = extractor.run_comprehensive_extraction_pipeline()
        
        if results['success']:
            print("\n🎉 1960 Multi综合数据集提取Pipeline执行成功!")
            print("=" * 70)
            ...
            print(f"  高置信度 (≥0.6): {conf_dist['high']:,} 个 ({conf_dist['high']/total_detections*100:.1f}%)")
            print(f"  良好置信度 (≥0.4): {conf_dist['good']:,} 个 ({conf_dist['good']/total_detections*100:.1f}%)")
